   ...
   if(!empty($custom_imgurl))
	{
		...
		$icons = (is_array(unserialize($sec_options['sfsi_custom_files']))) ? maybe_unserialize($sec_options['sfsi_custom_files']) : array();
		if(empty($icons))
		{   
			end($icons);
			$new=0;
...
function sfsi_deleteIcons()
{
   ...
   if(isset($_POST['icon_name']) && !empty($_POST['icon_name']))
   {
       ...
       $up_icons		= (is_array(unserialize($sec_options1['sfsi_custom_files']))) ? maybe_unserialize($sec_options1['sfsi_custom_files']) : array();
       $icons_links		= (is_array(unserialize($sec_options2['sfsi_CustomIcon_links']))) ? maybe_unserialize($sec_options2['sfsi_CustomIcon_links']) : array();
